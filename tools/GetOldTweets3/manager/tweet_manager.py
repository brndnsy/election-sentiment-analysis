...
class TweetManager:
    ...
    def getTweets(tweetCriteria, receiveBuffer=None, bufferLength=100, proxy=None, debug=False):
        ...
        for batch in n_batches:  # process all_usernames by batches
            refreshCursor = ''
            batch_cnt_results = 0
    ...
    # @staticmethod
    @sleep_and_retry
    @limits(calls=15, period=sixteen_mins)
    def getJsonResponse(tweetCriteria, refreshCursor, cookieJar, proxy, useragent=None, debug=False):
        ...
        if hasattr(tweetCriteria, 'lang'):
            ...
            urlLang = ''
        url = url % (urllib3.parse.quote(urlGetData.strip()), urlLang, urllib3.parse.quote(refreshCursor))
        # useragent = useragent or TweetManager.user_agents[0]
        useragent = random.choice(TweetManager.user_agents)
        ...
        if proxy:
            opener = urllib3.request.build_opener(urllib3.request.ProxyHandler({'http': proxy, 'https': proxy}),
                                                  urllib3.request.HTTPCookieProcessor(cookieJar))
        else:
            opener = urllib3.request.build_opener(urllib3.request.HTTPCookieProcessor(cookieJar))
        ...
        try:
            ...
            print("An error occurred during an HTTP request:", str(e))
            print("Try to open in browser: https://twitter.com/search?q=%s&src=typd" % urllib3.parse.quote(urlGetData))
            jsonResponse = response.read()
            # sys.exit()
            pass
