# -*- coding: utf-8 -*-
import json, re, datetime, http.cookiejar
import urllib.request, urllib.parse, urllib.error
from pyquery import PyQuery
# from .. import models
from tools.GetOldTweets3 import models
...
proxy_set = get_proxies()
proxy_pool = cycle(sample(proxy_set, int(round(len(proxy_set)) / 3)))
...
class TweetManager:
    ...
    @staticmethod
    def getTweets(tweetCriteria, receiveBuffer=None, bufferLength=100, proxy=next(proxy_pool), debug=False):
        ...
        user_agent = choice(TweetManager.user_agents)
        ...
        for batch in range(n_batches):  # process all_usernames by batches
            ...
            while active:
                ...
                for tweetHTML in tweets:
                    ...
                    # if batch_cnt_results > 14000 and not (batch_cnt_results >= tweetCriteria.maxTweets):
                    #     time.sleep(randint(0, 10))

                    if 0 < tweetCriteria.maxTweets <= batch_cnt_results:
                        active = False
                        break
    ...
    # @limits(calls=30, period=five_mins)
    @staticmethod
    @sleep_and_retry
    @limits(calls=randint(5, 20), period=2 * one_min)
    def getJsonResponse(tweetCriteria, refreshCursor, cookieJar, proxy, useragent=None, debug=False):
        ...
        # useragent = choice(TweetManager.user_agents)
        ...
        try:
            ...
            # time.sleep(one_min)
            time.sleep(randint(one_min, five_mins))
            if e.code == 429:  # too many requests error
                # response = opener.open(url)
                # jsonResponse = response.read()
                while True:
                    print("trying new proxy...")
                    # response = http.request('GET', 'http://example.com/', proxies, retries=Retry(10))
                    # response = requests.get(url, proxies={"http": proxy, "https": proxy}, retries=1)
                    response = requests.get(url, proxies={"http": proxy, "https": proxy})
                    if response.ok:
                        break
                    else:
                        time.sleep(sixteen_mins)
            ...
            elif e.code == 60 or e.code == 503:  # i.e. Operation timed out
                # time.sleep(sixteen_mins)
                proxy = next(proxy_pool)
                while True:
                    response = requests.get(url, proxies={"http": proxy, "https": proxy})
            ...
            try:
                ...
                # added end
                print("\nAn error occurred during an HTTP request:", str(e))
                print(
